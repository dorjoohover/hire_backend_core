...
const OUT_NAME = getArg(
  'out',
  `Hire_RawData_Assessment${ASSESSMENT_ID}_${new Date().toISOString().slice(0, 10)}.xlsx`,
);
...
// Минутаар өгөгдсөн үргэлжлэх хугацааг "Ц:ММ" (цаг:минут) хэлбэрт хөрвүүлнэ.
// Жишээ: 75 → "1:15", 45.7 → "0:46" (минутыг тоймлоно).
function fmtDuration(minutes: any): string | null {
  const n = num(minutes);
...
// question/questionCategory-ийн нэр нь rich-text editor-оос ирдэг тул
// "<p>...</p>" мэт HTML tag агуулдаг (жишээ: "<p>Та өдөр бүр хангалттай
// эрч хүчтэй байдаг уу?</p>"). Excel-д цэвэрхэн харагдуулахын тулд tag-уудыг
// цэвэрлэнэ.
...
async function main() {
  ...
  const answerRes = await client.query(
    ...
    SELECT
      ...
      -- questionAnswer.value ихэвчлэн NULL биш, харин '' (хоосон мөр) байдаг тул
      -- COALESCE дангаараа '' дээр зогсоод дараагийн (matrix/raw) утга руу шилждэггүй.
      -- NULLIF(..., '')-ээр хоосон мөрийг NULL болгож байж жинхэнэ fallback ажиллана.
      COALESCE(
        NULLIF(qa.value, ''),
        NULLIF(qm.value, ''),
        NULLIF(ua.value, '')
  ...
  const blockNames = [...new Set(blockPointRes.rows.map((r: any) => stripHtml(r.blockName)))].sort();
  const pivotMap = new Map<string, Record<string, number | null>>();
  for (const row of blockPointRes.rows) {
    const cleanBlockName = stripHtml(row.blockName) as string;
    if (!pivotMap.has(row.code)) pivotMap.set(row.code, {});
    pivotMap.get(row.code)![cleanBlockName] = num(row.totalPoint);
  }
  ...
  const examHeader = [
    ...
    'Урьсан огноо', 'Эхэлсэн огноо', 'Дууссан огноо', 'Үргэлжилсэн (мин)', 'Үргэлжилсэн (ц:мм)',
    'Овог', 'Нэр', 'Имэйл', 'Утас',
  ...
  const examRows = [
    ...
    ...examRes.rows.map((r: any) => [
      ...
      fmtDate(r.invitedAt), fmtDate(r.startedAt), fmtDate(r.finishedAt), num(r.durationMinutes), fmtDuration(r.durationMinutes),
  ...
  const answerRows = [
    ...
    ...answerRes.rows.map((r: any) => [
      ...
      stripHtml(r.blockName), stripHtml(r.questionName), num(r.questionOrder),
      stripHtml(r.answerValue), num(r.point),
    ]),
  ];
  ...
  const orgHeader = ['Байгууллага', 'Имэйл', 'Дуусгасан тоо', 'Дундаж хугацаа (мин)', 'Дундаж хугацаа (ц:мм)', 'Дундаж оноо'];
  const orgRows = [
    orgHeader,
    ...orgRes.rows.map((r: any) => [
      r.orgName, r.orgEmail, num(r.completedCount), num(r.avgDurationMinutes), fmtDuration(r.avgDurationMinutes), num(r.avgPoint),
    ]),
  ];
