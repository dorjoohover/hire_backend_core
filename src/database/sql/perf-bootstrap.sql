-- Performance bootstrap: runs idempotently on app startup (see database.module.ts).
-- Each statement must be safe to re-run (IF NOT EXISTS) and is executed
...
--    Avoids the repeated 3-4 table join that runs once per question
